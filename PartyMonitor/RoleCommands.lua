...
function SlashCmdList.PARTYROLES(msg)
    ...
    elseif command == "healers" then
        local healers = partyMonitor:GetHealers()
        if #healers == 0 then
            DEFAULT_CHAT_FRAME:AddMessage("|cff00ff00[Roles]|r No healers assigned")
        else
            DEFAULT_CHAT_FRAME:AddMessage("|cff00ff00[Roles]|r Healers:")
            for _, healer in ipairs(healers) do
                DEFAULT_CHAT_FRAME:AddMessage(string.format("  %s (%s)", healer.name, healer.role))
            end
        end
        
    elseif command == "dps" then
        local dps = partyMonitor:GetDPS()
        if #dps == 0 then
            DEFAULT_CHAT_FRAME:AddMessage("|cff00ff00[Roles]|r No DPS assigned")
        else
            DEFAULT_CHAT_FRAME:AddMessage("|cff00ff00[Roles]|r DPS:")
            for _, dpsPlayer in ipairs(dps) do
                DEFAULT_CHAT_FRAME:AddMessage(string.format("  %s (%s)", dpsPlayer.name, dpsPlayer.role))
            end
        end
